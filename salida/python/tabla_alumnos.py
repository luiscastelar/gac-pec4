...
FILE_LOGGIN = TAREA_PATH + 'app-alumnos.log'
logger = None


def create_alumnos_widgets(self):
    env = {
    ...
    columnas = ["id", "nombre", "apellidos", "edad"]

    root = self.root

    # Create the main widgets for the book management interface
    self.clear_widgets()
    root.title(f"Gestión de alumnos")
    ...
    tabla = db.readSimpleList("SELECT * FROM alumnos")
    logger.debug(tabla)
