...
class CRUD:
    ...
    def create_index_widget(self):
        ...
        titulo = ttk.Label(
            self.main_frame,
            text="Panel principal (DB: sqlite ➡️ UI: python)",
            font=("Segoe UI", 14, "bold")
        )
        titulo.grid(row=0, column=0, pady=(0, 15))

        # Botones en la ventana principal para abrir las ventanas hijas
        self.alumnos_button = ttk.Button(
            self.main_frame,
            text="alumnos",
            width=20,
            command=self.alumnos
        )
        self.alumnos_button.grid(row=1, column=0, pady=5)

        self.calificaciones_button = ttk.Button(
            self.main_frame,
            text="calificaciones",
            width=20,
            command=self.calificaciones
        )
        self.calificaciones_button.grid(row=2, column=0, pady=5)



        # Acciones de los botones (funciones callback)
    def alumnos(self):
        import tabla_alumnos
        self._abrir_ventana_hija(
            "Gestión de alumnos",
            tabla_alumnos.create_alumnos_widgets
        )

    def calificaciones(self):
        import tabla_calificaciones
        self._abrir_ventana_hija(
            "Gestión de calificaciones",
            tabla_calificaciones.create_calificaciones_widgets
        )
