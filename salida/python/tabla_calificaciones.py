...
FILE_LOGGIN = TAREA_PATH + 'app-calificaciones.log'
logger = None


def create_calificaciones_widgets(self):
    env = {
        'SERVER_DB': "db_in_sqlite.db",
        'PORT_DB': "np", 
        'USER_DB': "np",
        'NAME_DB': "db_in_sqlite.db",
        'PASS_DB': "np",
        'TIPO_DB': "sqlite"
    }
    columnas = ["id", "alumnoId", "valor"]

    root = self.root

    # Create the main widgets for the book management interface
    self.clear_widgets()
    root.title(f"Gestión de calificaciones")
    ...
    tabla = db.readSimpleList("SELECT * FROM calificaciones")
    logger.debug(tabla)
